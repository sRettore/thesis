...
def main(opts):
    ...
    # Set up dataloader
    train_dst, _ , val_dst, test_dst, n_classes = get_dataset(opts, rank=rank)
    ...
    if opts.step > 0:
        ...
        if (not os.path.exists(path)):  # and opts.name != 'EWC' and opts.name != 'MiB' and opts.name != 'PI' and opts.name != 'RW':
            if opts.task == '19-1' or opts.task == '15-5' or opts.task == '10-10':
                ...
                try:
                    ...
                    path_FT = f"logs/{opts.task}/{opts.task}_FT/{opts.task}-{opts.dataset}_FT//{opts.task}-{opts.dataset}_FT_{opts.step - 1}.pth"
                    copy(path_FT, path)
        ...
        if os.path.exists(path):
            ...
            load_ckpt(opts.net_pytorch, step_checkpoint, model, strict=False, vshapeCheck=False)
            ...
            load_ckpt(opts.net_pytorch, step_checkpoint, model_old, strict=True, vshapeCheck=False)
    ...
    if opts.test and opts.ckpt is None:
        opts.ckpt = f"{logdir_full}/{task_name}_{opts.name}_{opts.step}.pth"    
    
    if opts.ckpt is not None and os.path.isfile(opts.ckpt):
        checkpoint = torch.load(opts.ckpt, map_location="cpu")
        load_ckpt(opts.net_pytorch, checkpoint, model, strict=True, vshapeCheck=True)
    ...
    while cur_epoch < opts.epochs and TRAIN:
        ...
        if (cur_epoch + 1) % opts.val_interval == 0:
            ...
            '''if opts.step != 0:
                val_test_loss, val_test_score, ret_test_samples = trainer.validate(loader=test_loader, metrics=val_metrics,
                                                                    world_size=world_size, logger=logger)
                logger.print("Done validation on Test set")
                logger.info(f"End of Validation_test {cur_epoch + 1}/{opts.epochs}, Validation_test Loss={val_test_loss[0] + val_test_loss[1]},"
                            f" Class Loss={val_test_loss[0]}, Reg Loss={val_test_loss[1]}")
                logger.info(val_metrics.to_str(val_test_score))'''
