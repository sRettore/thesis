...
class TrainerUDA:
    def __init__(self, model, model_old, device, opts, trainer_state=None, classes=None, logdir=None):
        ...
        args_uda = {
            ...
            'unkd' : opts.uda_unkd,
            'group_msq' : opts.group_msq,
            'new_msq' : opts.new_msq,
            'lkd_features_target': opts.lkd_features_target,
            'lfmsq': opts.uda_lfmsq,
            'lfIWmsq': opts.uda_lfIWmsq
            }
        ...
        self.lossTarget = LossTargetHandler(args_uda, num_classes = self.num_classes, old_classes = self.old_classes, val=opts.fix_old_cl)
        
        if opts.multi:
            self.lossLowSource = LossTargetHandler(args_low_source, applySoftMax=False)
            self.lossLowTarget = LossTargetHandler(args_low_target, applySoftMax=False)
        ...
        self.ret_intermediate_target = self.lossTarget.require_intermediate()
    ...
    def train(self, world_size, cur_epoch, iterations, optim, source_loader, target_loader, scheduler=None, print_int=10, logger=None,
        ...
        self.lossTarget.reset_dict()
        if self.multi:
            self.lossLowSource.reset_dict()
            self.lossLowTarget.reset_dict()
        lreg = torch.tensor(0.)
        
        source_loader.sampler.set_epoch(cur_epoch)
        target_loader.sampler.set_epoch(cur_epoch)
        ...
        for cur_step, ((source_images, source_labels, indices), (target_images, _, _)) in enumerate(zip (source_loader, target_loader)):
            features_old = None    
            features = None  
            ...
            if self.multi:
                loss_tot_source += self.lossLowTarget.train_batch(cur_epoch, cur_step, low_outputs, prediction=labels)
            ...
            images = target_images.to(device)#, dtype=torch.float32)            
            if self.lossTarget.require_model_old() and self.model_old is not None:
                with torch.no_grad():
                    outputs_old = self.model_old(images, ret_intermediate=self.ret_intermediate_target)  
                    if isinstance(outputs_old, tuple):
                        if self.ret_intermediate_target:
                            features_old = outputs_old[2]
                        outputs_old = outputs_old[0]
            else:
                outputs_old = None

            outputs = model(images, ret_intermediate=self.ret_intermediate_target)                
            if isinstance(outputs, tuple):
                if self.ret_intermediate_target:
                    features = outputs[2]
                if self.multi:
                    lowUDA_outputs = outputs[1]
                outputs = outputs[0]

            loss_tot_target = self.lossTarget.train_batch(cur_epoch, cur_step, outputs, outputs_old=outputs_old, features=features, features_old = features_old)
            
            if self.multi:
                p_h = F.softmax(outputs, dim = 1)
                p_l = F.softmax(lowUDA_outputs, dim = 1)
                p_ens = (p_h+p_l)/2
                max_value, low_predictions = p_ens.max(dim=1)
                

                # y_ens = argmax_c p_ens if p_high > delta or p_low > delta 
                # mask where p_high > delta or p_low > delta is false
                mask_c = (p_h < self.delta) & (p_l < self.delta)
                # mask where the argmax_c p_ens value is located
                mask_v = (p_ens == max_value.unsqueeze_(dim=1))
                # get true if for c* = argmax_c, condition does not apply and ar 
                mask, _ = (mask_v*mask_c).max(dim=1)
                # where condition fails to apply at argmax_c p_ens set prediction to the ignore index 
                low_predictions[mask] = 255

                loss_tot_target += self.lossLowTarget.train_batch(cur_epoch, cur_step, p_l, prediction=low_predictions)

            if self.where_to_sim == 'GPU_server':
                from apex import amp
                with amp.scale_loss(loss_tot_target, optim) as scaled_loss:
                    scaled_loss.backward()
            else:
                loss_tot_target.backward()
            ...
            epoch_loss, reg_loss, interval_loss = self.lossTarget.output_values(epoch_loss, reg_loss, interval_loss)
            if self.multi:
                epoch_loss, reg_loss, interval_loss = self.lossLowTarget.output_values(epoch_loss, reg_loss, interval_loss)            
            ...
            if (cur_step + 1) % print_int == 0:
                ...
                logger.debug("Loss made of:   "+self.lossSource.lossText()+self.lossTarget.lossText(prefix='UDA-')+(self.lossLowTarget.lossText(prefix='MULTI-') if self.multi else '')+(f",   LREG: {lreg}" if self.regularizer_flag else ""))
                ...
                if logger is not None:
                    ...
                    self.lossTarget.addToLogger(logger, x, prefix='uda-')
                    if self.multi:
                        self.lossLowSource.addToLogger(logger, x, prefix='multi-source-')
                        self.lossLowTarget.addToLogger(logger, x, prefix='multi-target-')
    ...
    def validate(self, loader, metrics, world_size, ret_samples_ids=None, logger=None, vis_dir=None, label2color=None, denorm=None, print_int=0, epoch=0, valTarget=False):
        ...
        with torch.no_grad():
            for cur_step, (images, labels, indices) in enumerate(loader):
                ...
                if (self.lossSource.require_model_old() or self.lossTarget.require_model_old()) and self.model_old is not None:
                    with torch.no_grad():
                        outputs_old = self.model_old(images, self.ret_intermediate)
                        if isinstance(outputs_old, tuple):
                            if self.ret_intermediate:
                ...
                # validate target
                self.lossTarget.validate_batch(cur_step, outputs, outputs_old = outputs_old, features=features, features_old = features_old)
                class_loss, reg_loss, interval_loss = self.lossTarget.output_values(class_loss, reg_loss, interval_loss)
                ...
                if print_int >0 and (cur_step + 1) % print_int == 0:
                    ...
                    if logger is not None:
                        ...
                        logger.debug("Loss made of:   "+self.lossSource.lossText()+self.lossTarget.lossText(prefix='UDA-')+(f", lreg: {lreg}" if self.regularizer_flag else ""))
                        #print("Class Counter: ", self.classCounters)
                    else:
                        print(f"Validation, Batch {cur_step + 1}/{iterations},"
                                    f" Loss={interval_loss}")
                        print("Loss made of:   "+self.lossSource.lossText()+self.lossTarget.lossText(prefix='UDA-')+(f", lreg: {lreg}" if self.regularizer_flag else ""))
                        #print("Class Counter: ", self.classCounters)
                    interval_loss = 0.0   
