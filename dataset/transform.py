...
class PILBlur(object):
    ...
    def __repr__(self):
        format_string = self.__class__.__name__ + '(' + format(self.prob) + '\n)'
...
class NumpyNormalize(object):
    ...
    def __call__(self, img, lbl=None):
        ...
        img -= self.mean
        img = img.transpose((2, 0, 1)).copy() # (C x H x W)
        new_image = torch.from_numpy(img)
        if lbl is not None:           
            return new_image, torch.from_numpy(np.array(lbl, dtype=np.uint8))
        else:
            return new_image

    def __repr__(self):
        format_string = self.__class__.__name__ + '(' + format(self.mean) + '\n)'
        return format_string

class MaskImageLabels(object):
    """Filter labels from the target PIL image.

    Args:
        labels (list of ``int`` objects): list of labels that are used, other labels are set to zero.
    ...
    def __call__(self, img, lbl=None):
        if lbl is not None:           
            return img, lbl.point(lambda x: x if x in self.labels else 0)
        else:
            return img

    def __repr__(self):
        format_string = self.__class__.__name__ + '(' + format(self.labels) + '\n)'
        return format_string
...
class GaussianBlur(object):
    ...
    def __call__(self, img, lbl=None):
        ...
        """
        if lbl is not None:
            return self.blur(img), lbl
        else:
            return self.blur(img)

    def __repr__(self):
        return '{0}'.format(self.blur)
