...
class VOCSegmentation(data.Dataset):
    ...
    def getFilename(self, idx):
        return self.images[idx][0].split('/')[1].split('.')[0]
...
class VOCSegmentationIncremental(data.Dataset):
    def __init__(self,
        ...
        if label_filter is not None:
            full_transform = MaskImageLabels(label_filter)
        else:
            full_transform = None
        
        full_voc = VOCSegmentation(root, 'train' if train else 'val', is_aug=True, transform=full_transform)
